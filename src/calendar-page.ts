...
<script>
  ...
  // Custom in-widget task creator. Because GHL's create response can be slow,
  // we close instantly on submit and confirm by refreshing the calendar, so
  // the UI never hangs on "Creating...".
  function openCreate(prefillDate){
    ...
    createBtn.addEventListener("click", function(){
      ...
      closeModal();                       // close immediately so it feels instant
      ...
      fetch("/api/cal/task/create" + apiQS(), { method:"POST", headers:{"content-type":"application/json"}, body: JSON.stringify(payload) })
        ...
        .then(function(res){
          if(!res.ok){ setStatus(""); alert("Could not create task: " + JSON.stringify(res.j)); return; }
          setStatus("Task created"); calendar.refetchEvents();
          setTimeout(function(){ calendar.refetchEvents(); setStatus(""); }, 3000);
        })
        .catch(function(){ setStatus(""); calendar.refetchEvents(); alert("Network error creating task."); });
    });
  }
